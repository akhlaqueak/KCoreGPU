__global__ void selectNodesAtLevel8(unsigned int *degrees, unsigned int level, unsigned int V, 
...
int kcoreSharedMemBallot(Graph &data_graph){
    ...
    while(count < data_graph.V){
        ...
        selectNodesAtLevel8<<<BLK_NUMS, BLK_DIM>>>(data_pointers.degrees, level, 
                        data_graph.V, bufTails, glBuffers);

        processNodes8<<<BLK_NUMS, BLK_DIM>>>(data_pointers, level, data_graph.V, 
